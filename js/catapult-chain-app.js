...
$(function() {
	...
	function changeNetwork() {
		let host = $('#network > option:selected').text();
	...
	function getSelectVal() {
		...
		let sel;
		if (host !== null) {
			let found = $('#network > option').filter(function() {
				return $(this).text() === host;
			});
			if (found.length === 1) {
				sel = found.val();
	...
	let getSammy = function(){
		...
		var app = $.sammy('#main', function() {
			...
			var host = params.get("host") || "catapult48gh23s.xyz:3000";
			var apiHost = 'http://' + host;
			var wsHost = 'ws://' + host + '/ws';
